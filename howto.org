...
#+DATE: March 2019 (emacs-reveal 1.0.0 and later)
...
*** Install via Git
    ...
    - As quickstart, inspect this
      [[https://gitlab.com/oer/emacs-reveal-howto][Howto]]
      - Which embeds ~emacs-reveal~ as submodule and contains source
        ...
        - Install necessary Emacs packages
...
*** Emacs-reveal like a Package?
    ...
    :END:
    - You do not need to install ~emacs-reveal~ as submodule
    - The file ~emacs-reveal.el~ can be located anywhere
      - E.g., execute the following to obtain copy of ~emacs-reveal.el~
        - =git clone https://gitlab.com/oer/emacs-reveal.git=
      - Maybe add a line like this to =~/.emacs=
	...
	in default browser
      - For this to work
        1. Settings of ~emacs-reveal~ should be in effect
           (~emacs-reveal.el~ is loaded, e.g., as explained
           [[#non-standard-install][above]])
