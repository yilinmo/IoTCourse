...
** Building Presentations from Emacs
   1. Load [[emacs-reveal/reveal-config.el][reveal-config.el]] from
      your =~/.emacs=, e.g., with a line like this:
      =(load "/path/to/emacs-reveal/reveal-config.el")=
   2. Generate HTML presentation for visited
...
** On Sections
   ...
   - This slide is part of section [[#features][Some Presentation Features]]
     - We can link to slides, e.g., the [[Text Slide][the previous slide]]
