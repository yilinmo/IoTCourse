...
#+DATE: March 2020 (emacs-reveal 7.1.0 and later)
...
*** Installation of ~emacs-reveal~
    ...
    :END:
    - You can install ~emacs-reveal~ in a directory of your choice
      ...
      2. Add following lines to =~/.emacs=
         ...
         - =(require 'emacs-reveal)=
           - Beware! This changes values of other packages (~org-ref~,
             ~oer-reveal~) without warning
      3. Restart Emacs (installation of ~org-ref~ is offered, if necessary)

*** Alternative installation
    - Starting with version 7.1.0, ~emacs-reveal~ includes lots of
      components as Git submodules, which are activated by default
      - If you prefer to manage packages such as ~org-re-reveal~ and
        ~oer-reveal~ yourself, set
        ~emacs-reveal-managed-install-p~ to nil before restarting Emacs:
        - =(add-to-list 'load-path "~/.emacs.d/elpa/emacs-reveal")=
        - =(setq emacs-reveal-managed-install-p nil)=
        - =(require 'emacs-reveal)=
      - (Now subdirectories under ="~/.emacs.d/elpa/emacs-reveal"=
        remain empty)
...
*** Quickstart with ~emacs-reveal~
    ...
    - E.g., generate this howto
      1. Choose directories for howto and emacs-reveal, clone both
         - ~git clone https://gitlab.com/oer/emacs-reveal.git~
      ...
      2. Generate the HTML presentation from Org source ~howto.org~
         - ~emacs --batch --load elisp/publish.el~
         - Publication code needs to be able to locate ~emacs-reveal.el~
           - Code in ~elisp/publish.el~ tries (a) load path after
             adding =~/.emacs.d/elpa/emacs-reveal= (suggested on
             [[#git-install][previous slide]]) and (b) sibling
             directory ~emacs-reveal~
...
** Licensing
   - Starting with emacs-reveal 5.0.3 (and oer-reveal 2.0.2),
     presentations can show license information that is derived from
     SPDX headers of the [[https://reuse.software/][REUSE]] project
     - See [[#license][licensing slide]] at the end of this presentation
       - Information on that slide is derived from the following
...
,#+SPDX-License-Identifier: CC-BY-SA-4.0
     ...
     - License information is also embedded in machine-readable RDFa
       format
   - Macros to embed OER figures with (human- and machine-readable)
