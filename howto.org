...
** Installation
   ...
   - Two options
     1. Install via Git ([[#git-install][next slide]])
     2. Install from MELPA ([[#melpa-install][afterwards]])
...
*** Install via MELPA
    :PROPERTIES:
    :CUSTOM_ID: melpa-install
    :END:
    - [[https://melpa.org/#/getting-started][Getting started with MELPA]]
      - Add melpa, e.g.:
        - ~(add-to-list 'package-archives '("melpa" . "https://melpa.org/packages/"))~
        - ~(package-refresh-contents)~
    - Install ~emacs-reveal~
      - ~M-x package-install~, enter “emacs-reveal”
      - Note that ~emacs-reveal~ contains some resources (Org, CSS, HTML
        files)
        - To use them as in
          [[https://gitlab.com/oer/emacs-reveal-howto/blob/master/elisp/publish.el][elisp/publish.el]]
          of this Howto, you may want to create a symbolic link from
          your Org source directory to the installation directory of
          ~emacs-reveal~
          - See variable (~C-h v~) ~emacs-reveal-dir~
   ...
   1. Generate HTML presentation for visited
	...
	directory
        - I use
          ...
          ~ln -s public/reveal.ls~
        - For image grids, you may need: ~(setq emacs-reveal-export-dir "./")~
